...
config = load_yaml("./config/unet_config.yml")
...
# Set the transform
transform=A.Compose([A.Resize(config['data']['img_size'][0], config['data']['img_size'][1]),
...
model = UNet(input_dim=config['model']['input_dim'],
             num_class=config['model']['num_class']).to(device)
